...
class SpaceNnet8Dataset(Dataset):
    def __init__(self,
        ...
        csv_filename = os.path.join(
            self.cfg.PATH_FOLD_CSV, f'{cfg.csv_state}fold{fold}_seed{cfg.seed}_{self.phase}.csv')
                ...
                if self.cfg.thick.use and i in ['road', 'speed', 'flood']:
                    ...
                    if len(image.shape) <= 2:
                        ...
                        for channel in range(image.shape[0]):
                            ...
                            image[channel] = cv2.dilate(
                                image[channel], kernel, iterations=1)
...
class BCEDiceLoss(torch.nn.Module):
    ...
    def forward(self, y_pr, y_gt):
        loss = self.raito * self.bce_criterion(y_pr, y_gt) + (
            1 - self.raito) * self.dice_criterion(torch.sigmoid(y_pr), y_gt)
        return loss
...
class MultiBCEDiceLoss(torch.nn.Module):
    ...
    def forward(self, y_pr, y_gt):
        ...
        loss_dice = (1 - self.raito) * \
            self.dice_criterion(torch.sigmoid(y_pr), y_gt)
...
class MultiBCETverskyLoss(torch.nn.Module):
    def __init__(self, raito=0.5, mode='multilabel'):
        ...
        self.raito = raito
        self.bce_criterion = torch.nn.BCEWithLogitsLoss()
        self.tvrsky_criterion = smp.losses.TverskyLoss(
            mode=self.mode, log_loss=False)
    ...
    def forward(self, y_pr, y_gt):
        ...
        if self.mode == 'multiclass':
            ...
            y_gt = torch.argmax(y_gt, dim=1)
        loss_dice = (1 - self.raito) * \
            self.tvrsky_criterion(torch.sigmoid(y_pr), y_gt)
...
class FocalDiceLoss(torch.nn.Module):
    def __init__(self, raito=0.5, mode='multilabel', gamma=2.0, normalized=False,):
        ...
        self.raito = raito
        self.focal_criterion = smp.losses.FocalLoss(
            mode=mode, gamma=gamma, normalized=normalized)
        self.dice_criterion = DiceLoss()

    def forward(self, y_pr, y_gt):
        loss = self.raito * self.focal_criterion(torch.sigmoid(y_pr), y_gt) + (
            1 - self.raito) * self.dice_criterion(torch.sigmoid(y_pr), y_gt)
        return loss
...
class SoftBCEDiceLoss(torch.nn.Module):
    def __init__(self, raito=0.5, smooth_factor=0.1):
        ...
        self.raito = raito
        self.bce_criterion = smp.losses.SoftBCEWithLogitsLoss(
...
class BCELovaszLoss(torch.nn.Module):
    def __init__(self, raito=0.5, mode='multilabel', per_image=False):
        ...
        self.raito = raito
        self.bce_criterion = torch.nn.BCEWithLogitsLoss()
        self.dice_criterion = smp.losses.LovaszLoss(
            mode=mode, per_image=per_image)
...
class SpaceNet8Model(LightningModule):
    ...
    def __share_step(self, batch, mode):
        ...
        if torch.rand(1)[0] < self.cfg.mixup and mode == 'train':
            ...
            loss_build = self._criterion(build, target_a[:, 0, ...].unsqueeze(dim=1)) * lam + \
                (1 - lam) * self._criterion(build,
                                            target_b[:, 0, ...].unsqueeze(dim=1))
            ...
            # loss
            loss_build = self._criterion(
                build, labels[:, 0, ...].unsqueeze(dim=1))
    ...
    def __share_epoch_end(self, outputs, mode):
        ...
        for target in ['build', 'speed', ]:
            for out in outputs:
                exec(
                    f"losses_{target}.append(out[f'loss_{target}'].cpu().detach().numpy())")
...
if __name__ == '__main__':
    ...
    class CFG(object):
        ...
        csv_state: str = 'clean-v2_'  # area_, clean-v1_, clean-v2_ '', fulltrain_,
        ...
        # preprocess
        features: List[str] = ["preimg", "building",
                               "road", "roadspeed", "flood"]
        ...
        runname: str = "3090"
        group: str = f'3090_V6_{csv_state}_{model["architecture"]}_IMG{preprocess["input_size"]}_timm-regnety_032-dec192-4_fl-tr_b4'
        notebook: str = 'baseline_foundation.py'
    ...
    for fold in range(cfg.folds):
        ...
        wandb_logger = WandbLogger(
            ...
            project=cfg.project,
            group=cfg.group,
            tags=[f'fold{fold}', '3090', 'script',
                  'foundation', 'build', 'speed'],
            # entity='spaceshift',
        )

    ...
    for index, row in df.iterrows():
        ...
        if row["Object"] != "LINESTRING EMPTY":
            ...
            for i in range(0, geom.GetPointCount()-1):
                ...
                x0, y0 = pt1[0], pt1[1]
                x1, y1 = pt2[0], pt2[1]
                # print(x0, x1, dist, y0, y1, dist)
                x, y = np.linspace(x0, x1, dist).astype(np.int64), np.linspace(y0, y1, dist).astype(np.int64)
                # print(x, y)
            ...
            # nums is empty case i.e.  flood_arr[top:bot,left:right] == (0,0)
            if nums == []:
                nums = [0]

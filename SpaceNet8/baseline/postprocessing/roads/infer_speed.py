...
def get_edge_time_properties(mask, edge_data, conv_dict,
    ...
    if verbose:
        if np.sum(mask) > 2:
            gt = True
        logger1.info("mask shape: " + str(mask.shape))
        logger1.info("mask has pos vals: " + str(gt))
        logger1.info("multichannel: " + str(multichannel))
    ...
    for j,(x,y, dl_pix) in enumerate(zip(x_mids, y_mids, dls)):
        ...
        if verbose:
            ...
            logger1.info(f"  (x0, x1), (y0, y1) ({x0},{x1}), ({y0},{y1})")
...
def add_travel_time_dir(graph_dir, mask_dir, conv_dict, graph_dir_out,
    ...
    for i,image_name in enumerate(image_names):
        ...
        mask = np.moveaxis(mask, -1, 0) ######## this is added ######
...
if __name__ == "__main__":
    ...
    args = parse_args()
    eval_csv = args.eval_csv # only used for plotting...
    mask_dir = args.mask_dir # dir to the road speed prediction masks.
    graph_dir = args.graph_dir # this is read only. they are the graphs from the last step. graphs_nospeed
    graph_speed_dir = args.graph_speed_dir # new graphs with speed are written here.
    log_file = args.log_file

    test_data_refined_dir  = None # this is a directory of all original RGB images used during prediciton. with this we will need to get image names from the eval csv img_path, mask_path...
    skeleton_band = 7
    num_channels = 3
    log_to_console = 1
    speed_conversion_file = "/workdir/SpaceNet8/baseline/postprocessing/roads/speed_conversion_key.csv"
    num_classes = 8
    mask_prefix = ""
    ...
    console, logger1 = make_logger.make_logger(log_file, logger_name='log',
                                               write_to_console=bool(log_to_console))
    ...
    speed_conversion_file = speed_conversion_file
    ...
    if N_plots > 0:
        rgb_image_filepaths = []
        with open(eval_csv, newline='') as csvfile:
            reader = csv.DictReader(csvfile)
            for row in reader:
                rgb_image_filepaths.append(row["preimg"])
        ...
        if len(image_list) > N_plots:
            image_names = np.random.choice(rgb_image_filepaths, N_plots)
        else:
            image_names = sorted(rgb_image_filepaths)
        ...
        for i,image_path in enumerate(image_names):
            ...
            pkl_path = os.path.join(graph_speed_dir, image_name.replace("_preimg.tif",  "_roadspeedpred.gpickle"))
            ...
            _ = apls_plots.plot_graph_on_im_yuge(G, image_path, figsize=figsize, 
                             ...
                             verbose=False, super_verbose=verbose)

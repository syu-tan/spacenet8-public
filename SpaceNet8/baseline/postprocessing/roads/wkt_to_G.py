...
def parse_args():
    ...
    parser.add_argument("--wkt_submission",
                        type=str,
                        help="the csv file to read linestrings from. this was output from previous vectorize roads step.")
    parser.add_argument("--graph_dir",
                         type=str,
                         help="output directory to store simplified graphs. These graphs do not incorporate speed for the edges though")
    ...
    parser.add_argument("--min_subgraph_length_pix",
                        type=int,
                        default=20)
    parser.add_argument("--min_spur_length_m",
                       type=int,
                      default=10)
    parser.parse_args()
...
def get_node_geo_coords(G, im_file, fix_utm_zone=True, n_threads=12,
    ...
    if n_threads > 1:
        ...
        for i in range(len(params)):
            coords_dict_list.append(pixelToGeoCoord(params[i]))
...
def get_edge_geo_coords(G, im_file, remove_pix_geom=True, fix_utm_zone=True,
    ...
    if n_threads > 1:
        ...
        coords_dict_list = []
        for i in range(len(params)):
            coords_dict_list.append(convert_pix_lstring_to_geo(params[i]))
...
if __name__ == "__main__":
    ...
    # this basically reads the wkt from the LINESTRINGS stored in the csv produced from the vectorize roads or skeletonize script.
    # it saves new graphs to graph_dir (these new graphs are slightly different than those saved in vectorize roads or skeltonize script)
    ...
    for i, image_id in enumerate(image_ids):
        ...
        out_file = os.path.join(graph_dir, f"{os.path.basename(image_id).split('.')[0]}.gpickle")

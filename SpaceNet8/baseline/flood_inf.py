...
def make_prediction_png(image, postimage, prediction, save_figure_filename):
    ...
    grid = [[raw_im, prediction],[post_im, 0, 0]]
    ...
    for row in range(2):
        for col in range(2):
            ...
            if row==0 and col == 0:
                ...
                ax.imshow(mask, cmap='gist_rainbow_r', alpha=0.9)
...
if __name__ == "__main__":
    ...
    predictions = np.zeros((len(val_dataset),img_size[0],img_size[1]), dtype=np.float32)
